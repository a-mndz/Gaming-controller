...
- [~] **T1.4** Phone transport layer (Wi-Fi first): DatagramSocket sender coroutine, latest-frame-only, 120 Hz cap, keepalive. Socket tuning: small buffers, DSCP EF. — Done 2026-08-17: `UdpTransport` (HELLO/ACK, ping-RTT, receive drain) + `DeckEngine` 120Hz sender coroutine, APK builds; on-device verification pending.
- [~] **T1.5** Phone mDNS/NSD discovery + connect flow + saved host + **manual IP** + MulticastLock handling (NSD flaky on OEM skins). — Done 2026-08-17: server-side `MdnsAnnouncer` (Makaretu.Dns.Multicast, `_europad._udp` — verified wire-visible from a second browser process); Android `EuroPadDiscovery` NSD client + discovered-hosts list in picker + manual IP + connect flow; MulticastLock + saved-host persistence pending on-device pass.
- [x] **T1.6** ~~Gamepad deck UI: sticks, ABXY, D-pad, bumpers, triggers, Start/Select → InputFrame.~~ Done 2026-08-17: drag sticks (L/R), D-pad direction zones, A/B/X/Y/LB/RB/START/BACK press-hold-release controls wired to InputFrame; trigger sliders pending polish pass.
- [~] **T1.7** Phone HUD: RTT (median/5), loss %, transport icon, deck name, **slot # (P1–P4)**. — Done 2026-08-17: RTT (1s cadence) + connection status + slot (P#) shown in header; median/loss%/transport icon pending polish.
- [x] **T1.8** ~~Deck picker home screen (faceplates grid), Gamepad deck wired.~~ Done 2026-08-17: single-deck v1 picker (connect panel + discovered servers + Gamepad deck); full faceplates grid deferred to polish pass with remaining decks.
- [ ] **T1.9** Integration: connect → play a Steam game end-to-end. Run PRD §4 Phase-1 acceptance checklist. ✅ done when all 5 criteria pass.
